...
declare
    %rest:GET
    %rest:path("/e-gedsh/api/geo/json")
...
declare
    %rest:POST('{$data}')
    %rest:path('/e-gedsh/api/data/serialize')
    %rest:header-param("Content-Type", "{$content-type}")
...
function api:get-page($folder as xs:string?, $page as xs:string?, $content-type as item()*) {
    ...
    let $id :=  if(contains($page,'.')) then concat($global:base-uri,'/',substring-before($page,"."))
                else concat($global:base-uri,'/',$page)   
    let $data := if(api:get-tei($id) != '') then api:get-tei($id) else api:not-found()
    return $id(:cntneg:content-negotiation($data, $content-type, $path):)
};
